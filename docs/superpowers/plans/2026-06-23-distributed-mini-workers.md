...
**Goal:** 让 NAS 只做任务调度+数据存储，抓取下沉到 Mac mini（首台 mini1），多 mini 共享 NAS 的 PostgreSQL 队列与代理租约，互不撞 IP、健康度按节点隔离，第一阶段仅 `xiaokang` 租户走 mini 验证且不中断其余租户。
...
| `scripts/migrate_proxy_health_node.py` | ProxyHealth 数据迁移（回填 node='nas'） | 8 |
| `docker-compose.yml` | PG 暴露 Tailscale；NAS worker 加 BLOCKLIST + NODE_ID | 10 |
| `deploy/io.smartcrawler.worker.plist` | launchd 模板 | 12 |
| `scripts/mini_bootstrap.sh` | mini 装机 | 12 |
...
**Files:**
- Create: `scripts/migrate_proxy_health_node.py`

- [ ] **Step 1: 写迁移脚本**

新建 `scripts/migrate_proxy_health_node.py`：
...
```bash
git add scripts/migrate_proxy_health_node.py
...
- [ ] **Step 2: postgres 端口绑 Tailscale IP**

修改 `docker-compose.yml` 的 postgres 服务，取消注释 ports 并绑 Tailscale IP：
...
```bash
docker exec smart-crawler python -m scripts.migrate_proxy_health_node \
...
<dict>
  ...
  <dict>
    <key>SC_ENV_FILE</key><string>/Users/solvea/.smart-crawler.env</string>
  </dict>
...
- [ ] **Step 2: worker 支持 SC_ENV_FILE 加载**
...
echo "bootstrap 完成。下一步：写 $USER_HOME/.smart-crawler.env，再装 launchd。"
```
...
Expected: 结尾打印 `bootstrap 完成`。

- [ ] **Step 5: 写 mini env 文件**

在 mini1 上创建 `/Users/solvea/.smart-crawler.env`（权限 600），`X` 替换为 xiaokang 的 workspace_id（查询见下）：

```bash
# 先查 workspace_id（在 NAS 上）：
# docker exec smart-crawler-pg psql -U smart_crawler -d smart_crawler -t \
#   -c "SELECT id FROM workspaces WHERE name='xiaokang' OR slug='xiaokang';"

ssh solvea@100.75.94.90 'umask 077; cat > ~/.smart-crawler.env <<EOF
...
EOF
chmod 600 ~/.smart-crawler.env'
```

- [ ] **Step 6: 装 1 个 launchd worker 并启动**

```bash
ssh solvea@100.75.94.90 'sed "s/__N__/1/g" ~/smart-crawler/deploy/io.smartcrawler.worker.plist \
  > ~/Library/LaunchAgents/io.smartcrawler.worker-1.plist
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/io.smartcrawler.worker-1.plist
launchctl kickstart -k gui/$(id -u)/io.smartcrawler.worker-1'
```

- [ ] **Step 7: 验证 mini1 领到 xiaokang 的 job**
...
Expected: `worker` 列为 `US-macmini1-1`，status 推进到 running/success。
...
```
Expected: 出现 `US-macmini1` 行（与 `nas` 行并存、独立）。
...
## Task 12: 【mini1·阶段3】放开 scheduled + 起第 2 个 worker + 24h 观察
...
```
Expected: scheduled 行的 worker 也是 `US-macmini1-*`。

- [ ] **Step 2: 起第 2 个 worker**

```bash
ssh solvea@100.75.94.90 'sed "s/__N__/2/g" ~/smart-crawler/deploy/io.smartcrawler.worker.plist \
  > ~/Library/LaunchAgents/io.smartcrawler.worker-2.plist'
# 注意：worker-2 的 WORKER_ID 需不同。env 文件用同一份 → WORKER_ID 会重复。
# 解决：plist 的 EnvironmentVariables 里覆盖 WORKER_ID=US-macmini1-2，
# 或为每个 worker 单独 env 文件 SC_ENV_FILE 指向不同文件。
```

调整：为 worker-2 单独建 `~/.smart-crawler-2.env`（仅 WORKER_ID 不同：`US-macmini1-2`，NODE_ID 仍 `US-macmini1`），plist 的 SC_ENV_FILE 指向它。然后：

```bash
ssh solvea@100.75.94.90 'launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/io.smartcrawler.worker-2.plist'
```
...
```
Expected: 同时出现 `US-macmini1-1` 和 `US-macmini1-2`，且 proxy_health 仍只有一个 `US-macmini1` node 行（两 worker 共享健康视角）。

- [ ] **Step 3: 24h 观察检查表**

逐项确认：
- 数据完整性：xiaokang 站点 products_count 与历史持平或更好。
- 429 率：`SELECT failure_code, count(*) FROM crawl_jobs WHERE worker LIKE 'US-macmini1%' AND finished_at > now()-interval '24h' GROUP BY failure_code;` —— 无异常飙升。
...
- 健康隔离生效：对比 `nas` 与 `US-macmini1` 两 node 下同一 IP 的 status 可不同。
...
MINIS=("solvea@100.75.94.90")     # 后续追加更多 mini
