...
### 3.4 `DELETE /api/ondemand/jobs/{id}` — 删除单条记录

- 取 job;若 `job.workspace_id != 当前工作区` → 403;不存在 → 404
- 删除该 `OnDemandJob` 行。**只删任务记录,不删已入库的 Product/Review 数据**
  (那些数据可能被多次抓取/其它任务共享,且按 sku upsert,删记录不应连带删数据)。
- 返回:`{deleted: true, id}`

### 3.5 `DELETE /api/ondemand/jobs` — 清空本工作区历史

- 删除 `workspace_id == 当前工作区` 的全部 `OnDemandJob` 行(同样只删记录,不删商品/评论数据)。
- 返回:`{deleted: <删除条数>}`
...
- 表格列:时间 / 平台 / URL(截断)/ listing 数 / 评论数 / 状态 / **操作(删除)**
- 状态用色:success=绿、partial=黄、failed=红(复用现有 `.badge` 样式)
- 每行末尾一个删除按钮(🗑/「删除」),点击 → 确认 → 调
  `DELETE /api/ondemand/jobs/{id}` → 成功后从列表移除(本地剔除或重新 loadOndemandJobs)
- 列表区顶部一个「清空历史」按钮 → 二次确认 → 调 `DELETE /api/ondemand/jobs` → 清空列表
...
函数 `loadOndemandJobs()`、`toggleJobDetail(id)`、`deleteOndemandJob(id)`、
`clearOndemandJobs()`。均加入 setup() return。
...
- **单测**(`tests/test_ondemand_jobs.py`):
  ...
  - `DELETE /jobs/{id}` 删除单条(越权 403、不存在 404),且**不删 Product/Review 数据**
  - `DELETE /jobs` 清空本 workspace 记录,不影响他 workspace、不删商品/评论数据
...
7. 删除单条记录后该行消失;「清空历史」清掉本工作区全部记录;两种删除都**不影响**
   已入库的 Product/Review 商品/评论数据。
