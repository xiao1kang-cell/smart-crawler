...
4. **node 取值**：进程级常量 `NODE_ID`（env），**无需逐 job 透传**。
   ...
   - mini：`NODE_ID=US-macmini1`（或由 `WORKER_ID` 推导机器部分，去掉 `-1/-2` 序号）
...
NODE_ID=US-macmini1               # 代理健康按节点隔离（D4）；同台多 worker 共用此值
...
阶段2  mini1 装机，起 1 worker，WORKSPACE_ALLOWLIST=X、NODE_ID=US-macmini1；
       NAS 加 WORKSPACE_BLOCKLIST=X。手动触发租户 X 某站 → 看 job 被
       US-macmini1-1 领、数据回 NAS PG、proxy_leases 无撞车、proxy_health 出现
       node=US-macmini1 的独立行（不与 nas 行互相污染）。其余租户 NAS 照抓不误。
阶段3  放开 mini 接租户 X 的 scheduled（靠 workspace_sites 映射），观察 24h：
       数据完整性、429 率、proxy_leases 活跃数 ≤ max_concurrency、
       验证"NAS 标坏的 IP 在 mini1 上仍可用"（D4 隔离生效）。
