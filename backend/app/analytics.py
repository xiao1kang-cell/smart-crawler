...
def recompute_site(session: Session, site_name: str,
                   review_rate: float | None = None) -> dict:
    """重算某站点的商品 30 天估算 + 趋势日汇总。

    30 天销量只在同一 SKU 有至少两个带评论数的快照时计算；单快照只
    能证明“有评论信号”，不能证明 30 天增量，返回 insufficient_history
    供后台展示，不伪造销量。
    """
    rate = float(review_rate or _review_rate() or 0.025)
    ...
    products = {p.sku: p for p in
                session.query(Product).filter(Product.site == site_name)}

    estimated_skus = 0
    review_signal_skus = 0
    sufficient_history_skus = 0
    ...
    for sku, snaps in by_sku.items():
        ...
        if p is None:
            continue
        recent = [x for x in snaps
                  if x.date >= cutoff and x.review_count is not None]
        all_review_snaps = [x for x in snaps if x.review_count is not None]
        sales = 0
        if all_review_snaps:
            review_signal_skus += 1
        if len(recent) >= 2:
            sufficient_history_skus += 1
            delta = (recent[-1].review_count or 0) - (recent[0].review_count or 0)
            sales = max(0, round(delta / rate)) if delta > 0 else 0
        elif len(all_review_snaps) == 1:
            insufficient_history_skus += 1
        p.thirty_day_sales = sales
        p.thirty_day_revenue = round(sales * (p.sale_price or 0), 2)
        if sales > 0:
            estimated_skus += 1
            total_sales += sales
            total_revenue += p.thirty_day_revenue or 0

    # ---- 趋势日汇总 ----
    dates = sorted({h.date for h in history})
    session.query(Trend).filter(Trend.site == site_name).delete()
    ...
    return {
        "site": site_name,
        "trend_days": len(dates),
        "skus": len(by_sku),
        "review_signal_skus": review_signal_skus,
        "sufficient_history_skus": sufficient_history_skus,
        "insufficient_history_skus": insufficient_history_skus,
        "estimated_skus": estimated_skus,
...
def recompute(site_name: str) -> dict:
    ...
    with session_scope() as s:
        return recompute_site(s, site_name)
